...
for i in range(10):
    ...
    def function(X1, X2, X3):
        y = 7.1 + 9.5 * X1 + 7.9 * X2 + 4.9 * X3 + 1.5 * X1 * X1 + 0.9 * X2 * X2 + 9.7 * X3 * X3 + 1.6 * X1 * X2 + \
            0.1 * X1 * X3 + 3.8 * X2 * X3 + 4.9 * X1 * X2 * X3 + randrange(0, 10) - 5
        return y
    ...
    if Fp < f.ppf(q=0.9, dfn=F4, dfd=F3):
        print("Рівняння регресії адекватне при рівні значимості 0.1")
    else:
        print("Рівняння регресії неадекватне при рівні значимості 0.1")
